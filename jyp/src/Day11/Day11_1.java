...
public class Day11_1 {
	...
	public static void main(String[] args) {
		...
		// 예외처리 : 에러 [예외]가 발생했을때 해당 에러 처리 해주는 역할 [ 에러수정 x]
			// 목적 : 안전한 프로그램 
				// [ 코드상 오류가 아닌 사용자가 예상치 못한 오류를 발생시켰을 경우 프로그램 정상화 ]
			// 형태 : try { 예외가 발생할 것 같은 코드 } 
					// catch(예외클래스 객체명){ 예외가 발생했을 경우의 코드 }
					// finally{ 예외 상관없이 무조건 실행되는 코드 }
				// if : 이미 알고 있는 예외가 있을경우
				// try : 예상치 못한 예외가 있을 경우
			// 예외 클래스 : Exeption 예외 슈퍼클래스
		...
		// 예1) 배열의 크기가 부족했을경우
		try { // 예외가 발생할 것 같은 코드 
				// 1. 예외발생 하지 않으면 그대로 실행
				// 2. 예외 발생시 예외 발생 지점에서 catch 이동 
		String[] 문자열배열 = new String[3]; // 3개의 String 객체를 저장하는 배열 선언
		
		문자열배열[0] = "유재석";		// [ ] : 인덱스 0번부터 순차적으로 저장하는 순서 번호
		문자열배열[1] = "신동엽";
		문자열배열[2] = "강호동";
		System.out.println(문자열배열);
		문자열배열[3] = "서장훈";		// 코드상 문제 없음 // 예외 발생 -> catch 로 이동
		
		}
		catch(ArrayIndexOutOfBoundsException 예외객체) {
			System.out.println("예외 발생 이유 : " + 예외객체); // 예외발생했을때의 대체 코드
		}
		...
		try {
			...
			System.out.println("예외 발생 이유 :" + 예외객체2);
		}
		...
		System.out.println("문자열 [객체] 정보 : " + 문자3.toString());
			...
			System.out.println("예외 발생 이유 : " + 예외객체3);
		}
		// 예4) 여러개의 예외가 발생했을때
		try {
